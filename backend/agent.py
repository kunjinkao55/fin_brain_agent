...
5. 检查定增/增发相关数字的单位是否合理。如果"定增股数"超过总股本3倍（如"259万亿股"），标记为"❌数据单位异常"并要求回退到原始数据源重新提取。
6. 检查评分卡"合计"分是否等于各维度得分之和。将所有维度得分相加，与显示的合计分对比。若不等，标记为"❌评分合计计算错误"。
7. 检查"操作建议"是否包含可执行的具体交易计划。必须包含6要素：①具体价格区间(精确到0.1元) ②仓位比例(%) ③止损价(具体数字) ④持仓周期(月数) ⑤止盈目标价 ⑥减仓/清仓触发条件。若缺少≥2项，标记为"⚠️操作建议空洞-缺少可执行要素"。若全是"逢低布局/择机介入/控制仓位"等模糊措辞而无具体数字，同样标记。
