...
collected_data 中已包含该股票最近5条公告——分析时必须逐条检查：
1. 是否有定增/配股/可转债预案? → 如果有，计算摊薄后EPS(旧股本/新股本×旧EPS)，前瞻PE必须基于摊薄后EPS。目标价下调幅度≈摊薄比例。
2. 是否有重大合同/股权变动/业绩预告/减持增持? → 这些事件可能改变估值假设或投资逻辑。
3. 如果定增规模超过市值10%，必须在风险中标注"大额融资摊薄风险"，在估值判断中明确区分"定增前PE"和"定增后PE"。
4. 股价大跌时，先检查公告——是不是定增/减持引发的稀释恐慌? 不要把有明确原因的大跌简单归因为"市场情绪悲观"。
