...
7. 检查"操作建议"是否包含可执行要素。请仔细阅读报告中的"[操作建议]"、"[止损]"、"[执行状态]"和"[综合结论]"段落（特别是"买入策略""卖出条件""预期收益""持仓周期"子项）。如果这些段落已经包含了具体的价格数字、仓位百分比、止损价、持仓周期、止盈目标、减仓/清仓条件，则判定为"已满足"，不要标记为空洞。只有当这些段落全部缺失或全部使用"逢低布局""择机介入""控制仓位"等无数字措辞时，才标记为"⚠️操作建议空洞"。
