...
12 项守卫 + 6 条一致性不变量 + 数字回溯验证 + FCF 预警注入。当评分引擎的保守 PE 锚点与 LLM 的增长叙事冲突时，生成 `[框架分歧]` 段落同时呈现两套逻辑，而非强行压平。
...
### 双层验证
- **Code 层（Audit Engine）**：预检（评分、PE 存在性、股本变动-估值联动、一致性不变量×6、数字回溯）
...
| 测试 | 98 项测试（72 项 e2e + 26 项 golden 回归） |
