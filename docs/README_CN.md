...
12 项守卫 + 10 条一致性不变量 + 数字回溯验证 + FCF 预警注入。当评分引擎的保守 PE 锚点与 LLM 的增长叙事冲突时，生成 `[框架分歧]` 段落同时呈现两套逻辑，而非强行压平。
...
### 双层验证
- **Code 层（Audit Engine）**：预检（评分、PE 存在性、股本变动-估值联动、一致性不变量×10、数字回溯）
- **LLM 层（Critics 三路并行）**：语义审查（逻辑、财务解读、行业事实）
- **Code 预检**：所有确定性检查通过则 LLM 审计降级为"仅警告"模式

### RAG 知识系统
8 个行业模板覆盖半导体、电力/能源、医药、消费品、光模块/通信、制造/新能源、金融、地产/建筑。每个模板包含：行业周期、估值规律（含历史 PE 区间）、竞争格局、产业链位置、关键指标。ONNX MiniLM 本地嵌入（无需 API 调用）。检索结果带稳定引用、来源元数据和可选行业/代码/文档类型过滤；注入 Analyst 上下文时保留 `[RAG:...]` 标签，实时财务数据仍以工具取数为准。
...
- 公告扫描（20 条，三级优先级，定增检测+自动摊薄修正，业绩快报正文提取）
- 板块资金流向与市场宽度
...
- 可比公司 PE/PB 对比 + 一致预期前瞻 + 监测表（触发器/窗口/来源，向机构研报看齐）
- 估值参数全配置化（configs/scoring.json [估值守卫]，零硬编码）
...
- Audit Engine（LLM 9 项清单 + 代码 12 项守卫 + 代码预检 + 4 级递进）
- LLM 输出 grounding 幻觉拦截（Critic 发现数字可溯源过滤、Repair 新数字作废、股评转写句子级剔除）
- 处理标记系统（防止重试时重复处理）
...
| 测试 | 148 项测试（88 项 e2e + 28 项 golden 回归 + 15 项引擎 + 17 项插槽） |
