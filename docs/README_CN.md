...
9 项确定性检查 + FCF 预警注入。当评分引擎的保守 PE 锚点与 LLM 的增长叙事冲突时，生成 `[框架分歧]` 段落同时呈现两套逻辑，而非强行压平。
...
### 双层验证
- **Code 层（Audit Engine）**：9 项预检（数学一致性、单位有效性、字段完整性、时效性矛盾）
