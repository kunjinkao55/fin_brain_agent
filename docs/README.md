...
### Harness 工程基座（17/17 守卫点全部激活）
| 层级 | 能力 |
|------|------|
| 调用层 | 数据不足拦截、TTL 感知工具去重、交易时段判断、注定失败拦截(3次→熔断) |
| 输出层 | 评分强制覆盖、估值水位 PE/PB 强制覆盖、Q1 现金流自动预警、输出格式校验+重试、对比范围限制、公司类型兜底、成长-估值匹配检查、**公告定增强制修正(单位感知+摊薄系数缓存)**、**稀释后依赖字段重算(安全边际/估值差距/买入区间)**、**评分维度代码兜底(行业前景/资金认可默认值)** |
| 校验层 | **校验审计Agent(7项检查含操作建议空洞检测)**、**代码级预检(跳过干净报告的重试)**、**处理标记跨重试传递(防二次稀释)**、4级递进处理(警告→外科修正→Analyst重推理→降级输出) |
