...
| **股本 SSOT + 事件分类** | 股本单一事实源 + IPO/定增/配股/送转事件分类器，IPO 绝不误判为定增，摊薄幂等 |
| **计算可回溯** | CalcTable 登记 + 报告数字回溯验证（分指标容差）+ 跨源比对 + 单位规范，LLM 不得发明数字 |
...
| **代码级审计** | 12 项守卫 + 一致性不变量×10 + FCF 预警 + 框架分歧检测，评分/估值/单位/时效性不交给 LLM |
| **幻觉拦截** | Critic/Repair/股评转写全部经 grounding 校验：数字必须可溯源至原文，否则丢弃/作废/句子级剔除 |
| **利润引擎解剖** | 主营构成拆分（毛利占比）+ SOTP 强制隔离阀 + 单一引擎锚定倾斜（分类引擎五支柱） |
| **估值呈现** | 三锚点合理区间 + 可比公司 PE/PB 对比 + 历史 PE band + 一致预期前瞻 + 预期差三方对照 |
| **监测表** | 风险触发器/观察窗口/来源 四列：解禁日程、财报披露日历、预告兑现、趋势支撑破位 |
| **双通道业绩快报** | 财报快源 `RPT_FCI_PERFORMANCEE` + 公告快报正文解析，正式财报真空期仍能覆盖最新业绩 |
| **情景估值校验** | 结构化 EPS/PE + 代码算术校验 + TTM/行业中枢双锚定钳制 + 伪情景检测重建 |
| **datasource tier 分层** | FREE / PREMIUM / INSTITUTIONAL 三级可插拔架构，预留付费数据源插槽 |
| **参数全配置化** | 估值层全部阈值集中于 `configs/scoring.json [估值守卫]`，零硬编码，改配置即改行为 |
...
python tests/test_data_slots.py             # 数据源插槽熔断链
```

当前测试覆盖（合计 **148 项**）：

- **88 项 e2e 测试**：编译、数据工具、评分一致性、估值、Harness 守卫、配置、输出一致性、报告质量守卫、结构化输出、LangGraph 路由、LLM Fallback、市场情绪、**Critic/Repair/股评幻觉拦截（grounding）**、财报期次纪律、历史 PE band、可比公司渲染、一致预期/监测表渲染、报告期动态推算
- **28 项托伦斯 golden 回归**：18 项历史错误模式回放 + 情景 EPS/PE 双锚定钳制，检出率 18/18
- **15 项分类引擎回归**：利润引擎解剖、SOTP 强制隔离阀、乘数矩阵三档、伪情景重建、INV-7/9/10 断路器
- **17 项数据源插槽测试**：熔断链与回退
- 关键真实数据冒烟：600131 业绩快报双通道接入、301583 全链路（事件分类→SSOT→TTM→次新股包→利润引擎）

最新验证结果：88/88 + 28/28 + 15/15 + 17/17 全部通过 ✅
...
│   ├── share_registry.py  # 股本单一事实源(SSOT) + TTM/EPS 统一入口 + BPS口径配对（FIX-01/02）
